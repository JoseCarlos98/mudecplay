...
export class RecordOcWarehouseExpense implements OnInit {
  ...
  ngOnInit(): void {
    this.photoId =
      this.getPhotoIdFromRoute();

    if (!this.photoId) {
      this.errorMessage =
        'No se encontró el identificador de la foto.';
  ...
  removeItem(
    ...
    if (
      this.itemsFA.length <= 1
    ) {
      return;
    }

    this.itemsFA.removeAt(
      index,
    );
  }
  ...
  onQuantityOrPriceChange(
    ...
    this.recalculateWarehouseAmount(
      group,
    );
  ...
  onPaymentBlur(
    ...
    /*
     * Si existe cualquier abono,
     * parcial o completo,
     * se debe capturar fecha.
     */
    if (
      amount > 0 &&
      payment > 0
  ...
  private recalculateWarehouseAmount(
    group: FormGroup,
  ): void {
          ...
          .map(
            (
              ...
              return {
                ...
                /*
                 * IMPORTANTE:
                 * El concepto pertenece
                 * a la partida.
                 *
                 * No usamos el concepto
                 * general de la O.C.
                 * como fallback.
                 */
                concept:
