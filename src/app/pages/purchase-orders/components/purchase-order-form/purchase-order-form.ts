...
} from '../../../../shared/services/workstation-printer.service';
...
export class PurchaseOrderForm
  ...
  ngOnInit():
    ...
    /*
     * Como el destino inicia en "direct",
     * el proyecto debe ser requerido
     * desde el inicio.
     */
    this.applyProjectValidator(
  ...
  get extraWorkHelpText():
    string {

    return 'Marca esta opción cuando la O.C. corresponda a trabajo extra del proyecto.';
  }
  ...
  get saveDisabled():
    ...
    return (
      ...
      this.isLocked ||
      (
        !this.isAuthorizedCorrectionMode &&
  ...
  get isAuthorizedCorrectionMode():
    boolean {
  ...
  get formStatusAlertMessage():
    ...
    if (
      ...
      return 'Esta O.C. está autorizada, pero no tiene gasto relacionado. Como administrador puedes corregir destino, factura, trabajo extra, monto, concepto y notas. El proyecto y solicitante se conservan bloqueados.';
    }
      ...
      .subscribe({
        ...
        error:
          (
            ...
            console.error(
              'Error guardando orden de compra:',
              err,
            );
          },
      });
      ...
      .subscribe({
        ...
        error:
          (
            ...
            console.error(
              'Error cargando formulario de O.C.:',
              err,
            );
          },
      });
  ...
  private patchOrder(
    ...
    this.currentExpenseLinksCount =
      Number(
        order.expense_links_count ??
        0,
      );


    this.isLocked =
      this.shouldLockOrderForm(
        order,
      );


    this.ensureRequesterOption(
      order,
    );
    ...
    this.form.patchValue({
      ...
      is_zero_amount_invoice:
        Boolean(
          order.is_zero_amount_invoice,
        ),


      zero_amount_reason:
        order.zero_amount_reason ??
        null,
  ...
  private buildPayload():
    ...
    const willHaveInvoice =
      String(
        raw.will_have_invoice,
      ) ===
      'true';
    ...
    const isZeroAmountInvoice =
      Boolean(
        raw.is_zero_amount_invoice,
      );


    const zeroAmountReason =
      String(
        raw.zero_amount_reason ??
        '',
      )
        .trim();
  ...
  private watchZeroAmountInvoice():
    void {
    ...
    this.form
      ...
      .subscribe(
        (
          ...
          this.applyZeroAmountInvoiceState(
            Boolean(
              value,
            ),
          );
  ...
  private applyZeroAmountInvoiceState(
    isZeroAmountInvoice:
      boolean,
  ):
    void {
  ...
  private mapRequestersToCatalog(
    requesters:
      entity.PurchaseOrderRequesterDto[],
  ):
    Catalog[] {
