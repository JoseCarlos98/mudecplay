...
@Component({
  selector: 'app-modal-purchase-cancel',
  standalone: true,
  imports: [
    CommonModule,
    ReactiveFormsModule,
  ...
  templateUrl: './modal-purchase-cancel.html',
  styleUrl: './modal-purchase-cancel.scss',
})
export class ModalPurchaseCancel {
  readonly data = inject<entity.PurchaseOrderResponseDto>(MAT_DIALOG_DATA);

  private readonly fb = inject(FormBuilder);
  private readonly dialogRef = inject(MatDialogRef<ModalPurchaseCancel>);
  ...
  cancelPurchaseOrder(): void {
    ...
    const payload: entity.CancelPurchaseOrderDto = {
    ...
    this.purchaseOrdersService
      .cancelPurchaseOrder(this.data.id, payload)
      .pipe(
        finalize(() => {
          this.saving = false;
        }),
      ...
      .subscribe({
        ...
        error: (err) => {
          console.error('Error al cancelar orden de compra:', err);
